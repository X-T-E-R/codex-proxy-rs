...
fn observe_turn_state(
    ...
    store.enqueue_observation(TurnStateObservation {
        ...
        attempt_index: Some(request.attempt_index),
...
#[derive(Clone, Copy)]
struct TurnStateRequestContext<'a> {
    request_id: &'a str,
    attempt_index: u32,
    client_turn_id: Option<&'a str>,
}
        ...
        loop {
            ...
            let next = match next {
                ...
                Err(mut failure) => {
                    ...
                    let metadata_changed = merge_reported_model_update(
                        response_metadata_updates.as_ref(),
                        &mut decoder,
                        &mut observation_state,
                    )
                    .await;
                    let observation_event = if rate_limits_changed
            ...
            if let Some(response_id) = decoder.response_id()
                && let Some(receipt) = pending_initial_turn_state_observation.take()
            {
                observe_received_turn_state(
                    turn_state_store.as_ref(),
                    response_transport,
...
async fn merge_reported_model_update(
    ...
    let reported_model = updates.lock().await.reported_model.clone();
    let Some(reported_model) = reported_model else {
        return false;
    };
    decoder.observe_reported_model(&reported_model);
    observation_state.observe_upstream_response_model(decoder.response_model())
}
